  ...
  methods: {
    ...
    editUserInfo() {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        // 判断
        if (res.meta.status !== 200)
          return this.$message.error("修改用户请求失败");
        this.editDialogVisible = false; //隐藏修改用户对话框
        this.getUserList(); //更新数据列表
        this.$message.success("用户信息修改成功"); //提示修改成功
      });
    },
    // 根据ID删除用户信息
    ...
    async setRole(userinfo) {
      ...
      this.setRoleDialogVisible = true; //显示对话框
    },
    // 点击按钮，分配角色
    async saveRoleInfo(){
